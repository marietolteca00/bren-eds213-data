...
whale <- read_csv(here::here('wk9','whale-entanglement.csv'))
...
file <- varToNumeric(
  obj = file,
  var = c("year",
          "month")   # <-- fill in the variable names
)
```
...
sdcInitial <- createSdcObj(
  dat         = file,                        # <-- which data frame?
  keyVars     = c("state","origin","county"),                     # <-- your QIDs
  excludeVars = c("case_id","long","lat","fishery_license"),      # <-- direct identifiers to exclude
  weightVar   = NULL,
  hhId        = NULL,
  strataVar   = NULL,
  pramVars    = NULL,
...
print(sdcInitial@risk$global$risk)
```
...
sdcInitial <- groupAndRename(
  obj    = sdcInitial,
  var    = "origin",          # <-- variable name
  before = c("recreational", "tribal"),    # <-- levels to merge
  after  = c("non-commercial")        # <-- new label
)
